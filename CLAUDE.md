...
- It is **resumable**: a `.progress.json` per region tracks completed top-level date ranges, and on restart it re-hydrates the accumulator from existing `daily.json`. To force a clean re-collect, delete the region's `daily.json` + `.progress.json` (re-hydration is skipped when `daily.json` is absent, avoiding any double-count).
- **Shipped JSON is trimmed to display windows** (`DAILY_KEEP_DAYS = 120`, `WEEKLY_KEEP_DAYS = 400`; monthly kept in full). This is mandatory, not cosmetic: full daily history is **>100 MB/region** and exceeds GitHub's hard per-file limit. The app only ever shows daily ≤90 days (3M), weekly ≤365 days (1Y), and monthly for ALL, so older fine-grained data is never rendered. Rollups are computed from the **full** history *before* trimming, so weekly/monthly stay accurate. On-demand is filtered of TITANS' `-1`/`0` "no price" sentinels and collapsed to change-points (a near-constant step function), shrinking it ~100x.
