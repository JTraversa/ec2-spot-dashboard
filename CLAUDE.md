...
There is no test suite. **Spot collection no longer lives here** — see the private sibling repo `../cloud-pricing-data` (collectors, raw event store, daily workflow that pushes daily JSON into this repo's `public/data/`).
...
SpotLake's TITANS API went dark ~2026-08-19 (404; site "under construction"). Spot data now comes from each provider's own history API via collectors in the **private sibling repo `../cloud-pricing-data`** (`collect-aws.cjs`, `collect-gcp.cjs`, `collect-azure.cjs`, `lib/spot-events.cjs`). Its daily workflow (03:17 UTC) checks this repo out, rebuilds `public/data/**/inst/*.json` + `meta.json` in place and pushes here (`chore(data): daily spot refresh`), which triggers Vercel. Raw events are the premium tier and never land in this public repo.
- **Public tier = one value per day.** Every `inst/*.json` row is `{date, avg}` and every `inst-pre/*.json` row is `[date, avg]` (since 2026-09-05). Open/high/low/close, event timing and per-AZ series are intraday information and exist only in the private raw store. Never add range fields back to the public JSON; the chart draws `avg` only and the CSV export has one price column.
- **`raw/<provider>/<region>/YYYY-MM-DD.tsv.gz` in cloud-pricing-data is the source of truth**: one immutable file per UTC day, one price-change event per line `ts \t az \t instance \t product \t price`, deduped on write. Everything else is derived and rebuildable.
- **Aggregation is time-weighted** (a price holds until the next event in its (instance, az) series; the final price is carried to collection time but capped by a per-provider `maxCarry`: AWS 10d, Azure 400d, GCP 400d; Azure and GCP list only SKUs still on offer, so a last price stays in force however old its effective date). AZs collapse by median (max high / min low). This is the same method as `inst-pre/` (Pauley), so no level step at the splice. TITANS-era rows were sample means and had their OHLC flattened by the old resume path — see below.
- **`rebuildInst`** replaces only daily rows the raw store covers, keeps earlier rows (TITANS 2024-02→2026-08-17, USC/ISI monthly archive), and re-rolls weekly/monthly. The cut-off is **per instance** (each type's own first raw event) when all raw months are read, and first-raw-month + maxCarry when a subset is. It was region-wide until 2026-09-05, which wiped most Azure SKUs' TITANS rows back to the oldest SKU's last change (2025-07). `updateMeta` regenerates `<provider>/meta.json[region]` from `inst/`.
- **Windows are short:** AWS and Azure serve 90 days. If the daily job breaks for >3 months that history is gone; the raw store is what makes the series continuous.
- **Azure quirks:** the `SpotResources` table is tenant-wide, so the Resource Graph request must NOT be scoped to a subscription (scoped = 0 rows; that was the 2026-09 blocker after the client secret). `properties.spotPrices[]` entries are `{effectiveDate, priceUSD}`, always stamped the 1st of a month; each SKU's list reaches back to whatever price covers the 90-day window, so first events range from 2025-07 to 2026-06. It also snapshots 28-day eviction rates to `<region>/eviction.json` (not yet charted).
